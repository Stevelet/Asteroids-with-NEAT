...
import added.DrawableMatrix;
import added.Random;
import added.Vector;

import java.util.ArrayList;

public class Player extends DrawableMatrix {
    Vector pos;
    Vector vel;
    Vector acc;

    double width = ScreenValues.width;
    double height = ScreenValues.height;

    int score = 0;//how many asteroids have been shot
    int shootCount = 0;//stops the player from shooting too quickly
    double rotation;//the ships current rotation
    double spin;//the amount the ship is to spin next update
    double maxSpeed = 10;//limit the players speed at 10
    ...
    Player() {
        ...
        acc = new Vector();
        rotation = 0;
        SeedUsed = (long) Math.floor(Random.random(1000000000));//create and store a seed
        Random.randomSeed(SeedUsed);

        //generate asteroids
        asteroids.add(new Asteroid(Random.random(width), 0, Random.random(-1, 1), Random.random (-1, 1), 3));
        asteroids.add(new Asteroid(Random.random(width), 0, Random.random(-1, 1), Random.random (-1, 1), 3));
        asteroids.add(new Asteroid(0, Random.random(height), Random.random(-1, 1), Random.random (-1, 1), 3));
        asteroids.add(new Asteroid(Random.random(width), Random.random(height), Random.random(-1, 1), Random.random (-1, 1), 3));
        //aim the fifth one at the player
        double randX = Random.random(width);
        double randY = -50 + Math.floor(Random.random(2))* (height+100);
    ...
    void move() {
        if (!dead) {
            ...
            if (AsteroidGameNeat.isOut(pos)) {//wrap the player around the gaming area
                loopy();
            }
        }
    }
    ...
    void checkTimers() {
        ...
        if (asteroidCount<=0) {//spawn asteorid
            ...
            if (replay) {//if replaying use the seeds from the arrayList
                Random.randomSeed(seedsUsed.get(upToSeedNo));
                upToSeedNo ++;
            } else {//if not generate the seeds and then save them
                long seed = (long) Math.floor(Random.random(1000000));
                seedsUsed.add(seed);
    ...
    void show() {
        if (!dead) {
            ...
            if (immortalCount >0 && Math.floor(((double)immortalCount)/5)%2 ==0) {//needs to appear to be flashing so only show half of the time
            } else {
                pushMatrix();
                ...
                if (boosting ) {//when boosting draw "flames" its just a little triangle
                    boostCount --;
                    if (Math.floor(((double)boostCount)/3)%2 ==0) {//only show it half of the time to appear like its flashing
    ...
    Player cloneForReplay() {
        ...
        clone.seedsUsed = (ArrayList<Long>) seedsUsed.clone();
        clone.brain.generateNetwork();
        return clone;
    }
    //---------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    void look() {
        ...
        for (int i = 0; i< vision.length - 1; i+=2) {
            direction = Vector.fromAngle(rotation + i*(Math.PI/8));
            direction.mult(10);
    ...
    double lookInDirection(Vector direction, int visionPos) {
        //set up a temp array to hold the values that are going to be passed to the main vision array

        Vector position = new Vector(pos.x, pos.y);//the position where we are currently looking for food or tail or wall
        double distance = 0;
        //move once in the desired direction before starting
        position.add(direction);
        distance +=1;
        Vector looped = new Vector(0, 0);
        ...
        while (distance< 60) {//!(position.x < 400 || position.y < 0 || position.x >= 800 || position.y >= 400)) {
            ...
            for (Asteroid a : asteroids) {
                if (a.lookForHit(position) ) {
                    ...
                    Vector towardsPlayer = new Vector(pos.x - asteroidHit.pos.x - looped.x, pos.y - asteroidHit.pos.y - looped.x);
