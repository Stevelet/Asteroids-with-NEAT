class original.Species {
  ArrayList<original.Player> players = new ArrayList<original.Player>();
  float bestFitness = 0;
  original.Player champ;
  ...
  original.Species(original.Player p) {
    players.add(p); 
  ...
  boolean sameSpecies(original.Genome g) {
    float compatibility;
    ...
    float largeGenomeNormaliser = 1;//g.genes.size();
  ...
  void addToSpecies(original.Player p) {
    players.add(p);
  }
  ...
  float getExcessDisjoint(original.Genome brain1, original.Genome brain2) {
    float matching = 0.0;
  ...
  float averageWeightDiff(original.Genome brain1, original.Genome brain2) {
    float matching = 0;
    float totalDiff= 0;
    for (int i =0; i <brain1.genes.size(); i++) {
      for (int j = 0; j < brain2.genes.size(); j++) {
  ...
  void sortSpecies() {
    ...
    ArrayList<original.Player> temp = new ArrayList<original.Player>();
  ...
  original.Player giveMeBaby(ArrayList<connectionHistory> innovationHistory) {
    original.Player baby;
    if (random(1) < 0.25) {//25% of the time there is no crossover and the child is simply a clone of a random(ish) player
      ...
      original.Player parent1 = selectPlayer();
      original.Player parent2 = selectPlayer();
